...
// const EventGroup = ({ events }: { events: CalendarEvent[] }) => {
//   return (
//     <div className="h-20 border-t last:border-b">
//       {events
//         .filter((event) => isSameDay(event.start, hour))
//         .map((event) => {
...
//           return (
//             <div key={event.pnl} className={cn("relative")}>
//               {event.title}
//             </div>
//           );
//         })}
      ...
      <div className="grid grid-cols-7 gap-px sticky top-0 bg-background border-b">
        {weekDays.map((day, i) => (
          <div
            key={day}
            className={cn(
              "mb-2 text-right text-sm text-muted-foreground mx-auto pr-2 ",
              [0, 6].includes(i) && "text-muted-foreground/50"
            )}
          >
            {day}
      ...
      <div className="grid overflow-hidden -mt-px flex-1 auto-rows-fr p-px grid-cols-7 gap-px">
        {monthDates.map((_date) => {
          const currentEvents = events.filter((event) =>
            isSameDay(event.date, _date.toISOString())
          );
          console.log(currentEvents, "current events");

          return (
            <div
              className={cn(
                "ring-2 ring-black p-2 text-sm text-muted-foreground bg-white ring-border overflow-hidden",
            ...
            >
              ...
              {currentEvents.map((event) => {
                return (
                  <div
                    key={event.pnl}
                    className="px-1 rounded font-semibold -mt-5 text-lg flex flex-col justify-center items-center gap-0"
                  >
                    <span className="flex-1 truncate text-gray-700 ">
                      {event.noOfTrades} Trades
                    </span>
                    <span
                      className={` ${
